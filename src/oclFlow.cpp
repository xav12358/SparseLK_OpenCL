...
class ocl_pyramid {
    public:
        ...
        cl_int init(int w, int h, const char *name = NULL);
...
template<int lvls, cl_channel_order channel_order, cl_channel_type data_type>
cl_int ocl_pyramid<lvls,channel_order,data_type>::init(int w, int h, const char *name )
{
    cl_int err;
                                                            ...
                                                            cl_kernel kernel_G )
{
    ...
    for( int i=0; i<lvls ; i++ ) {
        ...
        err = clSetKernelArg( kernel_G, argCnt++, sizeof(cl_mem), &(Ix.imgLvl[i].image_mem) );
        checkErr(err, __LINE__, "enqavant");
        err = clSetKernelArg( kernel_G, argCnt++, sizeof(cl_mem), &(Iy.imgLvl[i].image_mem) );
        checkErr(err, __LINE__, "enqavant");
        err = clSetKernelArg( kernel_G, argCnt++, sizeof(cl_mem), &scratchBuf.mem );
        checkErr(err, __LINE__, "enqavant");
        //printf("size %d %d %d\n ",Iy.imgLvl[i].w ,Iy.imgLvl[i].h,argCnt);
        err = clSetKernelArg( kernel_G, argCnt++, sizeof(cl_int), &(Iy.imgLvl[i].w) );
        checkErr(err, __LINE__, "enqavant");
        err = clSetKernelArg( kernel_G, argCnt++, sizeof(cl_int), &(Iy.imgLvl[i].h) );
        checkErr(err, __LINE__, "enqavant");
        //printf("size %d %d \n ",Iy.imgLvl[i].w ,Iy.imgLvl[i].h);
        //printf("enqavant\n");
        checkErr(err, __LINE__, "enqavant");
...
)
{
    ...
    for( int i=lvls-1; i>=0 ; i-- ) {
        ...
        clSetKernelArg( lkflow_kernel, 0, sizeof(cl_mem), &I.imgLvl[i].image_mem );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 1, sizeof(cl_mem), &Ix.imgLvl[i].image_mem );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 2, sizeof(cl_mem), &Iy.imgLvl[i].image_mem );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 3, sizeof(cl_mem), &G.imgLvl[i].image_mem );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 4, sizeof(cl_mem), &J_float.imgLvl[i].image_mem );
        checkErr(err, __LINE__, "enq");

        if( use_guess  ) {  // send previous level guesses if available
            clSetKernelArg( lkflow_kernel, 5, sizeof(cl_mem), &flowLvl[i+1].mem );
            clSetKernelArg( lkflow_kernel, 6, sizeof(cl_int), &flowLvl[i+1].w );
        } else { // if no previous level just send irrelevant pointer
            clSetKernelArg( lkflow_kernel, 5, sizeof(cl_mem), &flowLvl[0].mem );
            clSetKernelArg( lkflow_kernel, 6, sizeof(cl_int), &flowLvl[0].w );
        }

        clSetKernelArg( lkflow_kernel, 7, sizeof(cl_mem), &flowLvl[i].mem );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 8, sizeof(cl_int), &flowLvl[i].w );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 9, sizeof(cl_int), &flowLvl[i].h );
        checkErr(err, __LINE__, "enq");
        clSetKernelArg( lkflow_kernel, 10, sizeof(cl_int), &use_guess );
        checkErr(err, __LINE__, "enq");
        err = clEnqueueNDRangeKernel( cmdq, lkflow_kernel, 2, 0, 
...
cl_context initOCLFlow(GLuint vbo, int devId)
{
    ...
    cl_program lkflow_program = buildProgramFromFile(context,"/home/lineo/Bureau/Developpement/OpenCL_Flow/lkflow.cl");
    cl_program motion_programs = buildProgramFromFile(context,"/home/lineo/Bureau/Developpement/OpenCL_Flow/motion.cl");
    cl_program filter_programs = buildProgramFromFile(context,"/home/lineo/Bureau/Developpement/OpenCL_Flow/filters.cl");
    ...
    images[1] = ocl_load_image( context, "/home/lineo/Bureau/Developpement/OpenCL_Flow/data/minicooper/frame10.pgm", err );
    images[0] = ocl_load_image( context, "/home/lineo/Bureau/Developpement/OpenCL_Flow/data/minicooper/frame11.pgm", err );
...
float computeOCLFlow(int curr, int next)
{
    ...
    duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
    std::cout<<"Derivative Ix Iy duration : "<< duration <<'\n';
    start = std::clock();
    ...
    duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
    std::cout<<"G_Fill duration : "<< duration <<'\n';
    start = std::clock();

    J_float->convFill( *J, convert_kernel );
    duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
    std::cout<<"convFill duration : "<< duration <<'\n';
    start = std::clock();
