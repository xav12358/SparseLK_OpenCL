...
//                cv::imshow("Raw image",cv::Mat(frame));
//                cvWaitKey(-1);
    ...
    if( !image )
    {
        ...
        if( capture ) {
            ...
            printf("je suis la\n");
            cv::imshow("Raw image",cv::Mat(opencvImages[0]));
            cv::imshow("Raw image2",cv::Mat(opencvImages[1]));
            cvWaitKey(-1);
...
void renderScene(void) 
{
    ...
    if( capture)
    {
        ...
        glTexSubImage2D(GL_TEXTURE_RECTANGLE_NV, 0, 0, 0, gw, gh, GL_BGR, GL_UNSIGNED_BYTE, c_image->imageData );
    }
...
int main( int argc, char** argv )
{
    ...
    if( !capture ) {
        ...
        shrLoadPGMub( "/home/lineo/Bureau/Developpement/OpenCL_Flow/data/minicooper/frame10.pgm", (unsigned char **)&image_ub, &w, &h );
        glBindTexture(GL_TEXTURE_RECTANGLE_NV, tex);
